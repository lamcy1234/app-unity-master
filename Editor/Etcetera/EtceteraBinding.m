...
{
	if( [AppController respondsToSelector:@selector(registerForRemoteNotificationTypes:)] )
		[AppController performSelector:@selector(registerForRemoteNotificationTypes:) withObject:[NSNumber numberWithInt:types]];
	else
		NSLog( @"AppController does not respond to registerForRemoteNotificationTypes:. Did you remove the AppControllerPushAdditions file and forget to readd it?" );
}


int _etceteraGetEnabledRemoteNotificationTypes()
{
	if( [AppController respondsToSelector:@selector(enabledRemoteNotificationTypes)] )
	{
		NSNumber *num = [AppController performSelector:@selector(enabledRemoteNotificationTypes)];
		return [num intValue];
	}
	else
	{
		NSLog( @"AppController does not respond to enabledRemoteNotificationTypes. Did you remove the AppControllerPushAdditions file and forget to readd it?" );
	}
