...
public static class TransformExtensionMethods
{
    ...
    {
        UnityExtender.Instance.StartCoroutine (_MoveTo (tx, position, T, null));
    }
    ...
    {
        UnityExtender.Instance.StartCoroutine (_MoveTo (tx, position, T, OnFinish));
    }


    /// <summary>
    /// Smoothly move to a position over T seconds, yielding true if a raycast forward hits something.
    /// </summary>
    /// <param name="tx">
    /// A <see cref="Transform"/>
    /// </param>
    ...
    /// A <see cref="System.Single"/> The time taken to move to the target.
    /// </param>
    /// <param name="raycastDistance">
    /// Distance to raycast forward.
    /// </param>
    public static IEnumerable<bool> MoveTo (this Transform tx, Vector3 position, float T, float raycastDistance)
    {
        RaycastHit hit;
        while (_MoveTo (tx, position, T, null).MoveNext()) {
            if (Physics.Raycast (tx.position, tx.forward, out hit, raycastDistance)) {
                yield return true;
            }
            yield return false;
        }
    ...
    {
        UnityExtender.Instance.StartCoroutine (_RotateTo (tx, direction, T, null));
    }
    ...
    {
        UnityExtender.Instance.StartCoroutine (_ChangeRotation (tx, Quaternion.RotateTowards (tx.rotation, Quaternion.LookRotation (tx.right), degrees), T, null));
    }
    ...
    {
        UnityExtender.Instance.StartCoroutine (_ChangeRotation (tx, Quaternion.LookRotation (position - tx.position), T, null));
    }
    ...
    {
        UnityExtender.Instance.StartCoroutine (_ChangeRotation (tx, Quaternion.LookRotation (target.position - tx.position), T, null));
    }
    ...
    {
        UnityExtender.Instance.StartCoroutine (_RotateTo (tx, direction, T, OnFinish));
    }
    ...
    {
        UnityExtender.Instance.StartCoroutine (_ChangeRotation (tx, Quaternion.RotateTowards (tx.rotation, Quaternion.LookRotation (tx.right), degrees), T, OnFinish));
    }
    ...
    {
        UnityExtender.Instance.StartCoroutine (_ChangeRotation (tx, Quaternion.LookRotation (position - tx.position), T, OnFinish));
    }
    ...
    {
        UnityExtender.Instance.StartCoroutine (_ChangeRotation (tx, Quaternion.LookRotation (target.position - tx.position), T, OnFinish));
    }

    /// <summary>
    /// Smoothly rotate to face direction over T seconds, yielding true if a raycast forward hits something.
    /// </summary>
    /// <param name="tx">
    /// A <see cref="Transform"/>
    /// </param>
    ...
    /// Distance to raycast forward.
    /// </param>
    public static IEnumerable<bool> RotateTo (this Transform tx, Vector3 direction, float T, float raycastDistance)
    {
        RaycastHit hit;
        while (_RotateTo (tx, direction, T, null).MoveNext()) {
            if (Physics.Raycast (tx.position, tx.forward, out hit, raycastDistance)) {
                yield return true;
            }
            yield return false;
    ...
    /// <summary>
    /// Pivot on vertical axis to face a direction in degrees, yielding true if a raycast forward hits something.
    /// </summary>
    /// <param name="tx">
    /// A <see cref="Transform"/>
    /// </param>
    ...
    /// Distance to raycast forward.
    /// </param>
    public static IEnumerable<bool> PivotTo (this Transform tx, float degrees, float T, float raycastDistance)
    {
        RaycastHit hit;
        while (_ChangeRotation (tx, Quaternion.RotateTowards (tx.rotation, Quaternion.LookRotation (tx.right), degrees), T, null).MoveNext()) {
            if (Physics.Raycast (tx.position, tx.forward, out hit, raycastDistance)) {
                yield return true;
            }
            yield return false;
    ...
    /// <summary>
    /// Smoothly rotate to look at a position over T seconds, yielding true if a raycast forward hits something.
    /// </summary>
    /// <param name="tx">
    /// A <see cref="Transform"/>
    /// </param>
    ...
    /// Distance to raycast forward.
    /// </param>
    public static IEnumerable<bool> LookAt (this Transform tx, Vector3 position, float T, float raycastDistance)
    {
        RaycastHit hit;
        while (_ChangeRotation (tx, Quaternion.LookRotation (position - tx.position), T, null).MoveNext()) {
            if (Physics.Raycast (tx.position, tx.forward, out hit, raycastDistance)) {
                yield return true;
            }
            yield return false;
    ...
    /// <summary>
    /// Smooth rotate to look at a target over T seconds, yielding true if a raycast forward hits something.
    /// </summary>
    /// <param name="tx">
    /// A <see cref="Transform"/>
    /// </param>
    ...
    /// Distance to raycast forward.
    /// </param>
    public static IEnumerable<bool> LookAt (this Transform tx, Transform target, float T, float raycastDistance)
    {
        RaycastHit hit;
        while (_ChangeRotation (tx, Quaternion.LookRotation (target.position - tx.position), T, null).MoveNext()) {
            if (Physics.Raycast (tx.position, tx.forward, out hit, raycastDistance)) {
                yield return true;
            }
            yield return false;
    ...
    {
        UnityExtender.Instance.StartCoroutine (_MoveAlong (tx, path, T, null));
    }
    ...
    {
        UnityExtender.Instance.StartCoroutine (_MoveAlong (tx, path, T, lookAt, null));
    }
    ...
    {
        UnityExtender.Instance.StartCoroutine (_MoveAlong (tx, path, T, lookAlongPath, null));
    }
    ...
    {
        UnityExtender.Instance.StartCoroutine (_MoveAlong (tx, path, T, OnFinish));
    }
    ...
    {
        UnityExtender.Instance.StartCoroutine (_MoveAlong (tx, path, T, lookAt, OnFinish));
    }
    ...
    {
        UnityExtender.Instance.StartCoroutine (_MoveAlong (tx, path, T, lookAlongPath, OnFinish));
    }

    /// <summary>
    /// Smoothly move along a path over T seconds, yielding true if a raycast forward hits something.
    /// </summary>
    /// <param name="tx">
    /// A <see cref="Transform"/>
    /// </param>
    ...
    /// Distance to raycast forward.
    /// </param>
    public static IEnumerable<bool> MoveAlong (this Transform tx, IList<Vector3> path, float T, float raycastDistance)
    {
        RaycastHit hit;
        while (_MoveAlong (tx, path, T, null).MoveNext()) {
            if (Physics.Raycast (tx.position, tx.forward, out hit, raycastDistance)) {
                yield return true;
            }
            yield return false;
    ...
    /// <summary>
    /// Smoothly move along a path over T seconds, looking at a transform, yielding true if a raycast forward hits something.
    /// </summary>
    /// <param name="tx">
    /// A <see cref="Transform"/>
    /// </param>
    ...
    /// Distance to raycast forward.
    /// </param>
    public static IEnumerable<bool> MoveAlong (this Transform tx, IList<Vector3> path, float T, Transform lookAt, float raycastDistance)
    {
        RaycastHit hit;
        while (_MoveAlong (tx, path, T, lookAt, null).MoveNext()) {
            if (Physics.Raycast (tx.position, tx.forward, out hit, raycastDistance)) {
                yield return true;
            }
            yield return false;
    ...
    /// <summary>
    /// Smoothly move along a path over T seconds, optionaly looking forward along the path, yielding true if a raycast forward hits something.
    /// </summary>
    /// <param name="tx">
    /// A <see cref="Transform"/>
    /// </param>
    ...
    /// Distance to raycast forward.
    /// </param>
    public static IEnumerable<bool> MoveAlong (this Transform tx, IList<Vector3> path, float T, bool lookAlongPath, float raycastDistance)
    {
        RaycastHit hit;
        while (_MoveAlong (tx, path, T, lookAlongPath, null).MoveNext()) {
            if (Physics.Raycast (tx.position, tx.forward, out hit, raycastDistance)) {
                yield return true;
            }
            yield return false;
